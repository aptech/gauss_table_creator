...
Optional add-on package adapters (separate source files, not wired into `ptModelFrom`/`ptTableFrom` since the underlying packages may not be installed). Each file is self-contained per the `pubtable.dec`/`QARDL_SDF_INCLUDED` note above; load the optional library together with pubtable in one statement, e.g. `library maxlikmt, pubtable;`:
